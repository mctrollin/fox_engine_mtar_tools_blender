...
@dataclass
class FoxDataHeader:
    # Placeholder for Fox data header found in various BT templates.
...
class StringData2:
    ...
    @classmethod
    def read(cls, br: BinaryIO) -> 'StringData2':
        # Read EntryCount then that many StrCode32 entries
        data = br.read(4)
        if len(data) < 4:
            raise EOFError('Unexpected EOF while reading StringData2 count')
        count = struct.unpack('<I', data)[0]
        entries = []
        for _ in range(count):
            raw = br.read(4)
            if len(raw) < 4:
                raise EOFError('Unexpected EOF while reading StringData2 entry')
            entries.append(struct.unpack('<I', raw)[0])
        return cls(entries=entries)
    
    def write(self, bw: BinaryIO) -> None:
        """Write StringData2 to binary stream."""
        # Write entry count
        bw.write(struct.pack('<I', len(self.entries)))
        
        # Write entries
        for entry in self.entries:
            bw.write(struct.pack('<I', entry))
