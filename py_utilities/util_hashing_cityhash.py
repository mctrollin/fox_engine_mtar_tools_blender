...
def strcode32(text: str) -> int:
    ...
    Hashes the given string as-is without any extension stripping.  This is the
    canonical low-level primitive — it matches the HashWrangler ``FoxEngine.StrCode()``
    reference exactly.  Use :func:`strcode32_path` when the input is a file path
    whose extension should be stripped first.

    Used to hash animation track names, event names, bone names, and other animation-
    related identifiers in GANI and MTAR animation binary formats.

    This implements the HashWrangler.FoxEngine.StrCode() algorithm:
    https://github.com/TinManTex/HashWrangler/blob/master/HashWrangler/Hashing/FoxEngine.cs

    Args:
        text: String to hash (animation name, track name, bone name, event name, etc.).
              Passed through unchanged — no extension stripping is performed.
    ...
    Example:
        ...
        strcode32("track.gani") → uint32 hash of "track.gani" (dot included)
    ...
    """
    # Prepare seeds for CityHash64WithSeeds
    ...
    # Encode string with latin-1, append null terminator (matches C# behavior)
    data = _encode(text + "\0")

    # Hash using CityHash64WithSeeds
    hash64 = city_hash_64_with_seeds(data, seed0, seed1)

    # Mask to 48-bit (Fox Engine compatibility), then cast to 32-bit
...
def strcode32_path(text: str, remove_extension: bool = True) -> int:
    """Hash a file path string with StrCode32, optionally stripping the extension.

    Convenience wrapper around :func:`strcode32` for cases where the input is a
    file path that may carry an extension (e.g. ``"track.gani"``).  The extension
    stripping behaviour matches HashWrangler's default ``remove_extension=True``
    for path inputs.

    Use bare :func:`strcode32` when the input is already a plain name (bone name,
    event name, etc.) that must be hashed verbatim.
    ...
    Returns:
        32-bit StrCode32 hash of the (optionally stripped) string.

    Example:
        strcode32_path("track.gani") → uint32 hash of "track"
        strcode32_path("track.gani", remove_extension=False) → uint32 hash of "track.gani"
    """
    if remove_extension:
        dot_idx = text.find('.')
        if dot_idx >= 0:
            text = text[:dot_idx]
    return strcode32(text)
